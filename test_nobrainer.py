...
def test_sigmoid_property_basics(data):
    ...
    output = nobrainer.sigmoid(data)

    assert type(output) == type(np.array([1]))  # TODO: figure out a way to use isinstance
    for e in output:
...
@given(floats(min_value=-5, max_value=0),
       floats(min_value=0, max_value=5))
...
def test_NeuronLayer():
    layer = nobrainer.NeuronLayer(13, 17)
